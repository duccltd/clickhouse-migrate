    ...
    fn into(self) -> MigrationFile {
        ...
        if REGEX.is_match(&file_name) {
            ...
            return if matches.get(1).is_none() && matches.get(2).is_none() {
                MigrationFile::new(&file_name, &content, None)
            } else {
                ...
                let version = match matches.get(2) {
                    ...
                    None => return MigrationFile::default(&file_name, &content)
                };

                MigrationFile::new(name, &content, Some(version))
            }
        } else {
...
impl MigrationFile {
    pub fn new(name: &str, content: &str, version: Option<i32>) -> MigrationFile {
